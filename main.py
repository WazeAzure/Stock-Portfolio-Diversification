...
files=['ANTM.csv', 'ASII.csv', 'BBCA.csv', 'HEXA.csv', 'MDKI.csv']
dfs=[]

for file in files:
    folder='./data/Stock-Portfolio-Diversification - '
    temp=pd.read_csv(folder + file)
    temp.columns=['Date',file.replace('.csv','')]
    dfs.append(temp)

stocks = reduce(lambda left,right: pd.merge(left,right,on='Date'), dfs)
# stocks['year'] = stocks['Date'].apply(lambda x: x.split(' ')[0].split('/')[2])
# stocks['day'] = stocks['Date'].apply(lambda x: x.split(' ')[0].split('/')[1])
# stocks['month'] = stocks['Date'].apply(lambda x: x.split(' ')[0].split('/')[0])
stocks['Date'] = pd.to_datetime(stocks['Date'])
# print(stocks.shape)
stocks = stocks.set_index('Date').resample('M').last()
...
def hist_return(months):
    ...
    # print(stocks.dtypes)
    # print(stocks.iloc[-1,1:])
    # print(stocks.iloc[-mon-1,1:])
    # print(stocks.iloc[-1,1:] - stocks.iloc[-mon-1,1:])
    for mon in months:
        # print((stocks.iloc[-1,1:] - stocks.iloc[-mon-1,1:])/(stocks.iloc[-mon-1,1:]))
        temp=(stocks.iloc[0,1:] - stocks.iloc[mon-1,1:])/(stocks.iloc[mon-1,1:])
...
print(gen_mc_grid(5, 5, 100, 50))
print("====================")
print(gen_mc_grid_with_NA_loc(5, 5, 100, 50,range(10)))
...
# TAKE INITIAL POPULATION
n=len(files) # Number of stocks = 6
...
# standard deviation historical return
...
print(mean_portfolio_return(population[0]))
...
print(var_portfolio_return(population[0]))

# risk free factor
rf= 0.0697

# fitness function of portfolio
...
print(fitness_fuction(population[7]))
...
print(Select_elite_population(population, frac=0.3))
print([fitness_fuction(x) for x in population][:3])
...
def mutation(parent):
    ...
    child=parent.copy()
    n=np.random.choice(range(len(files)),2)
    while (n[0]==n[1]):
        n=np.random.choice(range(len(files)),2)
    child[n[0]],child[n[1]]=child[n[1]],child[n[0]]
    return child
print(mutation(population[1]),population[1])
...
print(Arithmetic_crossover(population[2],population[3]))
...
elite=Select_elite_population(population)
print(next_generation(100,elite,Arithmetic_crossover)[:3])
...
while ((Expected_returns < 0.30 and Expected_risk > 0.0005) or iteration <= 40) and iteration <= 200:
...
[print(hist_stock_returns.columns[i],':',elite[0][i]) for i in list(range(len(files)))]
